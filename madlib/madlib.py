...
def parse(string):
    """Parse a string into a sequence of parts
    """
    seq, string = _parse_sequence(string)
    assert string == "", "leftovers"
    return seq

def _parse_sequence(string, stops=[]):
    seq = Sequence()
    while string != "":
        if string[0] in stops:
            break
        if string.startswith("{"):
            a, string = _parse_alternate(string[1:])
            seq.add(a)
        else:
	    t, string = _parse_string(string, stops + ["{"])
	    seq.add(t)
    return (seq, string)

def _parse_alternate(string):
    a = Alternate()
    while string != "":
        s, string = _parse_sequence(string, ["|", "}"])
        a.add(s)
        if string.startswith("}"):
            break
        string = string[1:]
...
def _parse_string(string, stops):
    substr = ""
    while string != "":
        if string[0] in stops:
            break
        substr += string[0]
        string = string[1:]
    return (Text(substr), string)

def expand(seq, chooser=random.choice):
    """Expand a madlib sequence
    """
    collector = []
    seq.appendTo(collector, chooser)
...
class Sequence(object):
    ...
    def appendTo(self, stream, chooser):
        for part in self.parts:
            part.appendTo(stream, chooser)

class Text(object):
    """A part that's a text block
    """
    def __init__(self, text):
        self.text = text

    def appendTo(self, stream, chooser):
        stream.append(self.text)

class Alternate(object):
    """A part that represents alternatives of parts
    """
    def __init__(self):
        self.parts = []
    ...
    def appendTo(self, stream, chooser):
        if len(self.parts):
            chooser(self.parts).appendTo(stream, chooser)
